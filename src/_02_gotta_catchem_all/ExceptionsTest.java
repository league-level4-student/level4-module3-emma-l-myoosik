...
class ExceptionsTest {
	...
	public void testDivideByZeroException() {
		double dividend = 18.5;
		double divisor = 6.5;
		
		double quotient = 0.0;
		
		try {
			quotient = em.divide(dividend, divisor);
			assertEquals(quotient, 18.5/6.5);
			
			divisor = 0.0;
		} catch(IllegalArgumentException e) {
			e.printStackTrace();
			assertEquals(quotient, 18.5/0.0);
		}
	}
	...
	public void testReverseString() {
		String string = "perseverance";
		
		try {
			assertEquals("ecnarevesrep", em.reverseString(string));
			
			string = "";
		} catch (IllegalStateException e) {
			e.printStackTrace();
			assertEquals("", em.reverseString(string));
		}
	}
